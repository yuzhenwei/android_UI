...
def RunTest():
    ...
    print (now)
    report_path = './data/' + now + '.html'
    print(report_path)
    with open(report_path,"wb") as f1:
        description = '环境 ：{0} 版本：{1}'.format('安卓', '7.0')
        runner = HTMLTestRunner(stream=f1, title=u'自动化测试报告', description=description, verbosity=2,tester=u'李玲')
